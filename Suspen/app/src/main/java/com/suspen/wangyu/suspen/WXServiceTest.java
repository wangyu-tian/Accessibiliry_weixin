...
public class WXServiceTest extends AccessibilityService {
     ...
     * 拆红包类1
     */
    static final String WECHAT_RECEIVER_CALSS1 = "com.tencent.mm.plugin.luckymoney.ui.LuckyMoneyReceiveUI";
    /**
     * 拆红包类2
     */
    static final String WECHAT_RECEIVER_CALSS2 = "com.tencent.mm.plugin.luckymoney.ui.LuckyMoneyNotHookReceiveUI";
    /**
     * 红包详情类
     */
    ...
    private void onWindowStateChange(AccessibilityEvent event) {
        //查看是需要拆红包、红包已经被抢完
        if (WECHAT_RECEIVER_CALSS1.equals(event.getClassName())||
                WECHAT_RECEIVER_CALSS2.equals(event.getClassName())) {
    ...
    private void openHongBaoInChatPage(AccessibilityEvent event) {
        ...
        if (nodeInfo != null && nodeInfo.getChildCount() > 0) {
            ...
            List<AccessibilityNodeInfo> list2 =  nodeInfo.findAccessibilityNodeInfosByViewId("com.tencent.mm:id/aq5");//别人发的红包
            if (list1.size() > 0) {
                list.addAll(list1);
            }
            if (list2.size() > 0) {
                list.addAll(addNoGetList(list2));
            }

            //只抢最后一个
            for (int i = list.size() - 1; i >= 0; i--) {
                AccessibilityNodeInfo nodeInfo1 = list.get(i);
                if (nodeInfo1 != null) {
...
//                    }
     ...
     * @return
     */
    private List<AccessibilityNodeInfo> addNoGetList(List<AccessibilityNodeInfo> list2) {
        List<AccessibilityNodeInfo> list = new ArrayList<>();
        for(AccessibilityNodeInfo nodeInfo : list2){
            if(nodeInfo.getParent()!=null
                    &&nodeInfo.getParent().findAccessibilityNodeInfosByViewId("com.tencent.mm:id/aq6").size() == 0){
                while (nodeInfo != null) {
    ...
    private void processInHongBaoDialog() {
        ...
        if (nodeInfo != null) {
            ...
            if (!isHave) {
                List<AccessibilityNodeInfo> list = nodeInfo.findAccessibilityNodeInfosByViewId("com.tencent.mm:id/cyf");//nodeInfo.findAccessibilityNodeInfosByText("红包派完了");
